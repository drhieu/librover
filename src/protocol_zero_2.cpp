...
void Zero2ProtocolObject::motors_control_loop(int sleeptime) {
  ...
  while (true) {
    ...
    if (!estop_ &&
      ...
      motors_speeds_[REAR_LEFT_MOTOR] = duty_cycles.rl;
      motors_speeds_[REAR_RIGHT_MOTOR] = duty_cycles.rr;
...
void Zero2ProtocolObject::send_motors_commands() {
  robotstatus_mutex_.lock();
  int32_t v = static_cast<int32_t>(motors_speeds_[FRONT_LEFT_MOTOR] * 100000.0);
  unsigned char *payloadptr;
  uint8_t payload[5];
  ...
  robotstatus_mutex_.lock();
  // WIP
  v = static_cast<int32_t>(motors_speeds_[REAR_LEFT_MOTOR] * 100000.0);
