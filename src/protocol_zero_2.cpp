...
Zero2ProtocolObject::Zero2ProtocolObject(
  ...
  estop_ = false;
  motors_speeds_[FRONT_LEFT_MOTOR - 1] = MOTOR_NEUTRAL_;
  motors_speeds_[FRONT_RIGHT_MOTOR - 1] = MOTOR_NEUTRAL_;
...
void Zero2ProtocolObject::motors_control_loop(int sleeptime) {
  ...
  while (true) {
    ...
    if (!estop_) {
      ...
      robotstatus_mutex_.lock();
      motors_speeds_[FRONT_LEFT_MOTOR - 1] = duty_cycles.fl;
      motors_speeds_[FRONT_RIGHT_MOTOR - 1] = duty_cycles.fr;
...
void Zero2ProtocolObject::send_motors_commands() {
  ...
  robotstatus_mutex_.lock();
  // WIP
  v = static_cast<int32_t>(motors_speeds_[FRONT_LEFT_MOTOR - 1] * 100000.0);
  payload2[0] = COMM_CAN_FORWARD;
  payload2[1] = REAR_LEFT_MOTOR;
  ...
  write_buffer.clear();
  robotstatus_mutex_.lock();
  v = static_cast<int32_t>(motors_speeds_[FRONT_RIGHT_MOTOR - 1] * 100000.0);
  payload2[0] = COMM_CAN_FORWARD;
  payload2[1] = REAR_RIGHT_MOTOR;
  payload2[2] = COMM_SET_DUTY;
  payload2[3] = static_cast<uint8_t>((static_cast<uint32_t>(v) >> 24) & 0xFF);
